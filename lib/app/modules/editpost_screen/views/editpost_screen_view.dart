...
import '../controllers/editpost_screen_controller.dart';
...
class EditpostScreenView extends StatefulWidget {
	final String postId; // Declare the instance variable
	EditpostScreenView({required this.postId});

  @override
  _EditpostScreenViewState createState() => _EditpostScreenViewState();
}

class _EditpostScreenViewState extends State<EditpostScreenView> {
	//final EditpostScreenController editpostScreenController = Get.put(EditpostScreenController(Get.find<ApiService>()));
	final EditpostScreenController editpostScreenController = Get.find();
...
late PostData postData; // Define your postData variable

  @override
  void initState() {
    super.initState();
    // Initialize postData with appropriate values, e.g., from arguments or an API call
    postData = PostData(); // Initialize it according to your logic
  }
        ...
        return DefaultTabController(
          ...
          child: Scaffold(
            ...
            appBar: AppBar(
              ...
              title: const Text('Edit Post', style: TextStyle(fontSize: 18, color: Colors.black)),
              centerTitle: true,
            ),
            body: Obx(() {
				if (editpostScreenController.isFetchingData.value) {
					return Center(child: CircularProgressIndicator(valueColor: AlwaysStoppedAnimation<Color>(AppColor.purple),));					
				} else {
					final postData = editpostScreenController.postData.value;
					print("Files: ${postData.fetchedFiles}");
					//print("Total Files: ${postData.fetchedFiles.length}");
  
					// Ensure that the controller is only set once
					if (messageController.text.isEmpty && postData.text != null) {
						messageController.text = postData.text!;
					}
					if (priceController.text.isEmpty && postData.price != null && postData.price != 0) {
						priceController.text = postData.price.toString(); // Convert int to String
						hiddenPrice.value = priceController.text;  // Set hiddenPrice to the same value
					} else {
						print("Price is either empty, null, or 0");
					}
					print("Final priceController.text: ${priceController.text}");
					  ...
					  child: Form(
						...
						child: Column(
						  children: [
							...
							// Iterate through the selected files and print their details
							 // print("Selected Files Count: ${postData.fetchedFiles!.length}");
							// Check if postData.fetchedFiles is not null and has files
							if (postData.fetchedFiles != null && postData.fetchedFiles!.isNotEmpty)
							  Padding(
								...
								child: GridView.builder(
								  ...
								  itemCount: postData.fetchedFiles!.length,  // Use the length of fetchedFiles
								  itemBuilder: (context, index) {
									var file = postData.fetchedFiles![index];  // Access each FileData
									print("Selected Files-: ${file.filePath}");
									return buildFileItem(file, index, postData.fetchedFiles!);
								  },
								),
							  ),
							// If no files in postData, check selectedFiles
							  ...
							  Padding(
								...
								child: GridView.builder(
								  ...
								  itemBuilder: (context, index) {
									...
									print("Selected Files from Add-: $file");
									return buildFileItem(file, index, selectedFiles);
								  },
								),
							  ),
							// Show camera icon if both lists are empty
							if (postData.fetchedFiles == null && postData.fetchedFiles!.isEmpty && selectedFiles.isEmpty) 
								...
								Padding(
								  ...
								  child: ConstrainedBox(
									...
									child: textAreaFieldDynamic(
									  text: 'Write a edit post, drag and drop files to add attachments.',
									  width: screenWidth,
												  ...
												  product(
													...
													onPress: () {
													  ...
													  if (_modalFormKey.currentState?.validate() ?? false) {
														...
														hiddenPrice.value = priceController.text;
									  ...
									  Expanded(
										child: Obx(() {
										  return autoWidthBtn(
											...
											onPress: editpostScreenController.isLoading.value
												? null
												: () {
													if (_formKey.currentState!.validate()) {
													  // Call the submit function from the controller
													  editpostScreenController.submitPost(
  ...
  // Function to build each file item
Widget buildFileItem(dynamic  file, int index, List<dynamic > fileList) {
	Widget imageWidget;

  if (file is FileData) {
    // Use Image.network for URL in FileData
      ...
      Positioned(
        ...
        child: GestureDetector(
          onTap: () {
            ...
            final fileToRemove = fileList[index];

			//print("Checking if fetchedFiles contains: ${fileToRemove.filePath}");
			if (file is FileData) {
				print("Checking if fetchedFiles contains ID: ${fileToRemove.id}");
			  // Call the API
			  editpostScreenController.removeFileFromServer(fileToRemove.id);
			} else {
			  print("Not found in fetchedFiles.");
			}
            // Remove the file from the list regardless of its origin
            setState(() {
